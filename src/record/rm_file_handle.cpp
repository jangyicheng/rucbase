...
std::unique_ptr<RmRecord> RmFileHandle::get_record(const Rid& rid, Context* context) const {
    ...
    // 2. 初始化一个指向RmRecord的指针，赋值其内部的data和size
    RmRecord *record = new RmRecord(file_hdr_.record_size);
    char *slot_data = page_handle.slots + rid.slot_no * file_hdr_.record_size;
...
// 1. 获取当前未满的page handle
    ...
    // 3. 将buf复制到空闲slot位置
    memcpy(page_handle.slots + slot_no * file_hdr_.record_size, buf, file_hdr_.record_size);
...
void RmFileHandle::insert_record(const Rid& rid, char* buf) {
    RmPageHandle page_handle = fetch_page_handle(rid.page_no);
    memcpy(page_handle.slots + rid.slot_no * file_hdr_.record_size, buf, file_hdr_.record_size);
...
void RmFileHandle::update_record(const Rid& rid, char* buf, Context* context) {
    ...
    // 1. 获取指定记录所在的page handle
    // 2. 更新记录
    auto page_handle = fetch_page_handle(rid.page_no); // 获取指定记录所在的page handle
...
RmPageHandle RmFileHandle::fetch_page_handle(int page_no) const {
    ...
    if (page_no < 0 || page_no >= file_hdr_.num_pages) {
        throw std::runtime_error("PageNotExistError");
    }
